...
impl DirEntry {
    ...
    pub fn parse(data: &[u8]) -> FsResult<DirEntry> {
        ...
        let created_time_raw =
            u16::from_le_bytes([data[14], data[15]]) as u32;
        let created_date_raw =
            u16::from_le_bytes([data[16], data[17]]) as u32;
        let created_time =
            parse_datetime((created_date_raw << 16) | created_time_raw);

        let accessed_date_raw =
            u16::from_le_bytes([data[18], data[19]]) as u32;
        let accessed_time = parse_datetime(accessed_date_raw << 16);

        let modified_time_raw =
            u16::from_le_bytes([data[22], data[23]]) as u32;
        let modified_date_raw =
            u16::from_le_bytes([data[24], data[25]]) as u32;
        let modified_time =
            parse_datetime((modified_date_raw << 16) | modified_time_raw);
...
fn parse_datetime(value: u32) -> FsTime {
...
impl ShortFileName {
    ...
    pub fn parse(filename: &str) -> FsResult<ShortFileName> {
    ...
    let ext_part = ext_part.unwrap_or("");
    ...
    if name_part.bytes().chain(ext_part.bytes()).any(invalid) {
    ...
    name[..name_part.len()].copy_from_slice(name_part.as_bytes());
    ext[..ext_part.len()].copy_from_slice(ext_part.as_bytes());

    Ok(ShortFileName { name, ext })
}
}
